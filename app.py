...
with tab3:
    ...
    if check_btn:
        if not phone_input.strip():
            ...
            if "error" in result:
                ...
                line = str(result.get("line_type") or "").lower()
                ...
                if not result["valid"]:
                    ...
                    st.markdown(
                        "<div class='action-box'>⚠️ <b>VOIP / Virtual number</b> — commonly used to hide identity.</div>",
                        unsafe_allow_html=True,
                    )
                else:
                    st.markdown(
    ...
    helplines = [
        ...
        {"emoji": "🚨", "title": "Police Emergency",          "number": "112",   "link": None},
